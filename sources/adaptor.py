...
#            'full_restart': True,
    ...
    mod_map = {
        ...
        'linux.mesos.master' : {
            ...
            'type' : 'mesos',
            'require' : [
                {'linux.file': { 'path': '/tmp/install-java.sh', 'mode': '755', 'content': 'apt-get -y -q update\napt-get -y -q install software-properties-common htop\nadd-apt-repository -y ppa:webupd8team/java\napt-get -y -q update\necho oracle-java8-installer shared/accepted-oracle-license-v1-1 select true | sudo /usr/bin/debconf-set-selections\napt-get -y -q install oracle-java8-installer\n/usr/sbin/update-java-alternatives\n -s java-8-oracle' }},
                {'linux.cmd' : { 'cmd' : "which java && echo \"java installed\" || /bin/bash /tmp/install-java.sh", 'os': ["ubuntu","debian"] }},
                {'linux.cmd' : { 'cmd': "if [ ! -f /etc/zookeeper/conf/environment ];then cp /etc/zookeeper/conf_example/environment /etc/zookeeper/conf/environment; else echo '/etc/zookeeper/conf/environment existed'; fi", 'os': ["ubuntu","debian"]}},
